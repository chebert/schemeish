...
(for-macros
  (defstruct html-tag
    "Represents an html tag which can be rendered to text."
    name inner-tags-and-texts attribute-alist))
(export-definition
  (define html-tag? #'html-tag-p))
...
(export-definition
  (define (html-tag name inner-tags-and-texts attribute-alist)
    ...
    (make-html-tag :name name :inner-tags-and-texts inner-tags-and-texts :attribute-alist attribute-alist)))
...
(for-macros
  (defstruct markup
    "Represents a markup object. Constructor-form is a form which can re-construct the markup object.
...
(export-definition
  (define markup? #'markup-p))
(for-macros
  (defstruct (inline-markup (:include markup))
    "A markup object which is meant to be used inline."))
(export-definition
  (define inline-markup? #'inline-markup-p))

(for-macros
  (defstruct text-renderer
    "Describes a text-renderer's interface.
...
(export-definition
  (define text-renderer? #'text-renderer-p))
...
(export-definition
  (define (markup constructor-form render inline? html-tag)
    ...
    [(if inline? #'make-inline-markup #'make-markup) :constructor-form constructor-form :render-text render :html-tag html-tag]))
...
If there is more text to render after wrapping, passes it to render-inline."
	...
	(set! word-wrap? t))
      ...
      (define self
	(make-text-renderer :render-freshline render-freshline :render-newline render-newline
			    :render-prefix render-prefix :render-inline render-inline
			    :render-preformatted-text render-preformatted-text
			    :render-without-word-wrap render-without-word-wrap
			    :push-prefix push-prefix :pop-prefix pop-prefix
			    :copy-with-new-stream copy-with-new-stream))
      self)
...
| Row 2 C1 | Row 2 C2 |"
	...
	(foldl #'max 0 (map (lambda (markup) (element-width renderer markup)) column)))
    ...
    (unless (for-all (lambda (column-alignment) (member column-alignment '(:left :right :center))) column-alignments)
    ...
    (define (render renderer)
      (for-each (lambda (markup)
		  (cond
		    ((inline-markup? markup)
		     (render-markup markup renderer)
		     (render-without-word-wrap renderer (lambda (renderer) (render-inline renderer " "))))
